  ...
  return (
    <div className="mt-5 space-y-3">
      ...
      {products.map((product) => {
        ...
        const priceLabel = sku?.priceStatus === "unverified" ? "价格待核验" : "需匹配具体 SKU";
        return <article key={product.id} className="rounded-[20px] border border-black/5 bg-[#f7f8f9] p-4 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><h4 className="text-base font-medium">{product.name}</h4><p className="mt-1 text-xs text-neutral-500">{product.category}{sku ? ` · ${sku.spec} · ${sku.netContent}` : " · 茶品资料"}</p></div><span className="rounded-full bg-black px-3 py-1.5 text-xs font-medium text-white">{priceLabel}</span></div>
